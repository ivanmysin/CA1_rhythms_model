...
fig, axes = plt.subplots(nrows=28, ncols=2, gridspec_kw=gridspec_kw, constrained_layout=True, figsize=(10, 30))
    ...
    for celltype_idx, celltype in enumerate(plotting_param["neuron_order"]):
        for neuron_number in intracellular_group.keys():
            if celltype == intracellular_group[neuron_number].attrs["celltype"]:
                ...
                axes[celltype_idx + 5, 0].plot(t, Vm, label=celltype, color=plotting_param["neuron_colors"][celltype])

                axes[celltype_idx + 5, 1].text(0, 0.5, celltype)
                break
        ...
        axes[celltype_idx+14, 0].scatter(firings_x, firings_y, s=2, color=plotting_param["neuron_colors"][celltype],  label=celltype)
        axes[celltype_idx+14, 1].text(0, 0.5, celltype)
