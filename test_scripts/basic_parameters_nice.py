...
def get_basic_params():
    ...
    basic_params = {
        ...
        "duration": 10000,  # 10200, # 2500, # 700, # 2500, #, #  # 10 sec simulation time
        "step_duration": 0,
        ...
        "CellNumbers": {
            ...
            "Nca3_spatial": Nca3_spatial,  # 2000, # 3500,
        ...
        "CellParameters": {
            ...
            "mec": {
                ...
                "Rtheta": 0.3,
                "low_mu": -1.04,  # 5.24, # -0.5, # -0.8, # -1.75, # 0, #

                "Rgamma": 0.4,
                "high_mu": 0.0,

                "spike_rate": 10e7,  # 10000000.0,  # 100000.0,
                "latency": 10.0,
        ...
        "save_soma_v": {
            "pyr": [0, ], # [range(200, 300)],  # [0, ], #  [range(1400)]
            "pvbas": [0, ], #[range(30, 80)],  # [0, ], #
            "olm": [0, ], #[range(10)],  # [0, ], # [0, 1, 3],
            "cckbas": [0, ], #[range(10)],  # #[0, ],
            "ivy": [0, ], #[range(10)],  # [0, ], #[0, ],
            "ngf": [0, ], #[range(10)],  # [0, ], # [0, ],
            "bis": [0, ], #[range(10)],  # [0, ], # [0, ],
        ...
        "connections": {
            ...
            "ca3_spatial2pyr": {
                "gmax": 1000 * pyr_dx,  # 200* 1000 * pyr_dx,  # 16000.0, # 0.016,
                "gmax_std": 0.9,  # 0.002,
            ...
            # connection to pvbas
            "ca3_spatial2pvbas": {
                "gmax": 15 * pvbas_dx,  # 5 * pvbas_dx, # 10000, # 160, #7.0, # 0.7,
                "gmax_std": 0.2,
...
def get_object_params(Nthreads=1):
    ...
    pyr_coord_x = basic_params["pyr_coodinates"]  # np.zeros(Npyr) + 1500   #
    # pyr_coord_x[pyr_coord_x.size//2:] = np.nan

    pvbas_coord_x = basic_params["pvbas_coodinates"]  # np.cumsum( np.zeros(Npvbas) + 50)  #  50
    ca3_coord_x = basic_params["ca3_coodinates"]  #

    mec_grid_phases = np.linspace(-np.pi, np.pi, Nmec)  # rad  !!!!!!!!!!!
    # mec_grid_freqs = np.zeros(Nmec) + 0.333    # Hz
    ...
    for cell_idx, celltype in enumerate(cell_types_in_model):
        ...
        if cell_param["cellclass"] == "ArtifitialPlaceCell":
            ...
            neuron["cellparams"]["grid_freqs"] = basic_params["CellParameters"]["mec"][
                "grid_freqs"]  # next(mec_grid_freqs_iter)
    ...
    var_conns_on_pyr = basic_params["var_conns_on_pyr"]  # 1000.0
        ...
        for postsynaptic_cell_idx, post_celltype in enumerate(cell_types_in_model):
            ...
            if conn_name == "ca3_spatial2pyr":
                ...
                if not (np.isnan(pyr_coord)):
                    ...
                    dist_normalizer = np.exp(-0.5 * dist ** 2 / var_conns_on_pyr) / (
                ...
                if not (np.isnan(pyr_coord)):
                    ...
                    grid_centers = 1000 * prelib.get_grid_centers(grid_freq, grid_phase,
                                                                  basic_params["duration"] * 0.001)
                ...
                if not (np.isnan(pyr_coord)):
                    ...
                    dist_normalizer = np.exp(-0.5 * dist ** 2 / var_conns_on_pvbas) / (
                        np.sqrt(var_conns_on_pvbas * 2 * np.pi))
                ...
                dist_normalizer = np.exp(-0.5 * dist ** 2 / var_conns_pvbas2pvbas) / (
                    np.sqrt(var_conns_pvbas2pvbas * 2 * np.pi))
    ...
    for th_idx in range(Nthreads):
        ...
        OBJECTS_PARAMS[th_idx]["common_params"]["radius4piramids"] = np.sqrt(
            basic_params["CellNumbers"]["Npyr"] / basic_params["PyrDencity"]) / np.pi
